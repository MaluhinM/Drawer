try:  # Защита от 102-го
    ...
    from PIL import Image  # Сохранение изображения
    from keyboard import add_hotkey, block_key, hook  # Управление
    from tkinter.messagebox import showerror, showwarning, showinfo  # Создание окон ошибок, предупреждений и информирования
    from os.path import exists  # Проверка наличия путей
    from os import mkdir  # Создание папок
    ...
    other_keys: tuple = ('exit', 'reset_pencolor', 'reset_bgcolor', 'reset_heading', 'reset_position', 'clear', 'reset', 'pen', 'hide', 'undo', 'size+', 'size-', 'pencolor', 'bgcolor', 'fill', 'prtsc', 'prtsc_dir')  # Прочие функции программы
    ...
    def IntegrityCheck() -> 'Проверка целостности программы':  # Проверка целостности программы
        ...
        if not exists(data_folder): RaiseError(causes_keys[0], f'Отсутствует папка <{data_folder}>')  # Проверка наличия папки. В случае ее отсутствия - вызывается ошибка
        print('Выполнено')  # Вывод информации в консоль

        print('Проверка файла <figures.py>: ', end='')  # Вывод информации в консоль
        if not exists('figures.py'):
        ...
        for i in range(len(hotkeys)):  # Цикл проверки наличия путей
            if exists(hotkeys[i]): exist[i]: list = 1  # Наличие пути отмечается в списке
        ...
        for i in range(len(keys)):  # Цикл проверки
            for g in range(len(keys[i])):  # Проверка функций
                if exists(f'{hotkeys[i]}\\{keys[i][g]}.txt'): exist[n]: list = 1  # Если путь существует, то его присутствие отмечается в списке
        ...
        for i in range(len(vls)):  # Цикл проверки
            if exists(vls[i]): exist[i]: list = 1  # Если путь существует, то его присутствие отмечается в списке
        if 0 in exist:  # Если есть отсутствующие пути
            errors: list = []  # Объявление списка с ошибками
            for i in range(len(vls)):  # Цикл проверки
    ...
    def ResetPenColor():
        global curr_color
        turtle.color(curr_color := 'black')
    def ResetBgColor():
        global curr_bgcolor
        screen.bgcolor(curr_bgcolor := 'white')
    def RandomPenColor():
        global curr_color
        turtle.color(curr_color := '#' + ''.join(choices('abcdef0123456789', k=6)))
    def RandomBgColor():
        global curr_bgcolor
        screen.bgcolor(curr_bgcolor := '#' + ''.join(choices('abcdef0123456789', k=6)))
    ...
    def Fill():
        ...
        if fill_value:
            ...
            turtle.begin_fill()
    def PrtSc():
        if not exists(f'{data_folder}\Temp\\'): mkdir(f'{data_folder}\Temp\\')
        screen.getcanvas().postscript(file=f'{data_folder}\Temp\gui_image_grabbed.ps')
        if exists(f'{data_folder}\Temp\gui_image_grabbed.ps'):
            curr_time = strftime("%d.%m.%Y %H-%M-%S")
            if not exists(f'{data_folder}\ScreenShots\\'): mkdir(f'{data_folder}\ScreenShots\\')
            Image.open(f'{data_folder}\Temp\gui_image_grabbed.ps').save(f'{data_folder}\ScreenShots\\{curr_time}.png')
            print(f'Снимок экрана <{curr_time}> сохранен по пути <{data_folder}\ScreenShots\>')
            showinfo('Снимок экрана сохранен', 'Путь: <{}>\n\nНажмите <{}> для открытия папки'.format(f'{data_folder}\ScreenShots\\{curr_time}.png', hk['prtsc_dir']))
        else:
            print(f'Отсутствует файл по пути <{data_folder}\Temp\gui_image_grabbed.ps>')
            showwarning('Файл отсутствует', 'Путь: <%s>' % f'{data_folder}\Temp\gui_image_grabbed.ps')
    def PrtScDir():
        if not exists(f'{data_folder}\ScreenShots\\'): mkdir(f'{data_folder}\ScreenShots\\')
        Popen(f'explorer {data_folder}\ScreenShots\\')
    ...
    def start() -> None:  # Основная программа
        ...
        """
        global hk, curr_color, curr_bgcolor
        ...
        if exists(title_path): title_name: 'Имя окна' = open(title_path, 'r', encoding='utf-8').read().strip()  # Проверка присутствия пути и запись имени окна в переменную
        if not exists(title_path) or title_name in {''}:  # Если путь отсутствует или оно пустое
        ...
        if exists(speed_path): speed: 'Скорость' = open(speed_path, 'r', encoding='utf-8').read()  # Проверка присутствия пути и запись скорости в переменную
        if not exists(speed_path) or not speed.isdigit():  # Если путь отсутствует или значение скорости не число
        ...
        curr_color, curr_bgcolor = 'black', 'white'
        turtle.title(' • '.join(map(str, (title_name, speed, turtle.pensize(), curr_color, curr_bgcolor, x := '[Заливка]' if fill_value else '[Рисование]'))))
        ...
        for i in range(len(hotkeys)):  # Цикл записи клавиш в словарь
            if list(hotkeys)[i].replace(f'{hotkeys_folder}\\', '') == 'other':  # Если разделом клавиш является 'Прочее'
                for g in keys[i]:  # Цикл проверки состояния функций
                    ...
                    if not exists(f'{other_values}\\{g}.txt') or not open(f'{other_values}\\{g}.txt', 'r', encoding='utf-8').read() == '1':  # Если путь отсутствует или значением файла не является '1'
        ...
        add_hotkey(hk['reset_pencolor'], ResetPenColor)
        add_hotkey(hk['reset_bgcolor'], ResetBgColor)
        ...
        add_hotkey(hk['hide'], lambda: turtle.hideturtle() if turtle.isvisible() else turtle.showturtle())
        add_hotkey(hk['size+'], lambda: turtle.pensize(turtle.pensize()+1))
        add_hotkey(hk['size-'], lambda: turtle.pensize(turtle.pensize()-1) if turtle.pensize() > 1 else turtle.pensize(1))
        add_hotkey(hk['pencolor'], RandomPenColor)
        add_hotkey(hk['bgcolor'], RandomBgColor)
        add_hotkey(hk['fill'], Fill)
        add_hotkey(hk['prtsc'], PrtSc)
        add_hotkey(hk['prtsc_dir'], PrtScDir)

        hook(lambda event: turtle.title(' • '.join(map(str, (title_name, speed, turtle.pensize(), curr_color, curr_bgcolor, x := '[Заливка]' if fill_value else '[Рисование]'))) if event.event_type == 'down' else None))
        print('Выполнено')  # Вывод информации в консоль
        ...
        if len(figures) > 0:
            ...
            if len(figures) == 1:
                print(f'\nБыла активирована фигура <{figures[0].replace("figure_", "")}> на клавишу <{figures_keys[figures[0]]}>')
            else:
                print(f'\nБыли активированы фигуры ({len(figures)}):')
