...
from os import path, mkdir, rename  # Проверка наличия путей, создание папок и переименование файлов
...
from ast import parse  # Проверка синтаксиса
...
def IntegrityCheck() -> 'Проверка целостности системы':  # Проверка целостности программы
    ...
    print(f'Проверка файла <figures.py>: ', end='')  # Вывод информации в консоль
    if not path.exists(figures_py): RaiseError(causes_keys[0], 'Отсутствует файл <figures.py>')  # Проверка наличия папки. В случае ее отсутствия - вызывается ошибка
    print('Выполнено')  # Вывод информации в консоль
...
def RaiseError(title: 'Оглавление ошибки', txt: 'Текст ошибки', NeedReinstall = True) -> None:  # Вызов ошибки (принимает имя и текст ошибки)
    ...
    win: 'Окно' = Tk()  # Создание окна для сообщения о проблеме с целостностью программы
    win.resizable(False, False)  # Отключение возможности изменения размера
    win.title(title)  # Имя окна
    ReinstallText = '\n\nПереустановите программу' if NeedReinstall == True else ''
    Label(win, text=f'{txt}{ReinstallText}', font='Arial 15', bg='black', fg='red').pack()  # Создание и отрисовка лейбла с найденными ошибками и призыву к переустановке программы
    add_hotkey('Esc', lambda: win.destroy())  # Назначение клавиши Esc на завершение работы программы
...
def InstallDataPack(dp):
    folder = path.dirname(dp) if path.basename(dp)[:3] == 'dp_' or path.basename(dp)[:9] == 'datapack_' else RaiseError(causes_keys[4], f'Имя пакета данных по пути <{dp}>\nне начинается с <dp_> или <datapack_>', NeedReinstall=False)
    print('\nУстанавливаю пакет данных <%s>...' % dp.replace(f'{folder}\dp_', '').replace(f'{folder}\datapack_', '').replace('.txt', ''))
    ...
    print('Информация о пакете:\n' + info) if '\n' in info else print('Информация о пакете:', info)

    if 'code' in com.keys():
        open(figures_py, 'a', encoding='utf-8').write(f'\n{com["code"]}\n') if info == 'Отсутствует' else open(figures_py, 'a', encoding='utf-8').write(f'\n"""\n{info}\n"""\n{com["code"]}\n') if '\n' in info else open(figures_py, 'a', encoding='utf-8').write(f'\n# {info}\n{com["code"]}\n')
        print('Код пакета:\n' + com['code'])

    if 'type' in com.keys():
        try:
            print('Тип пакета:', com['type'].split()[0])
            if com['type'].split()[0].lower() == 'figure':
                open('{}\{}.txt'.format(figures_folder, 'figure_' + com['type'].split()[1]), 'w', encoding='utf-8').write('1')
                open('{}\{}.txt'.format(hotkeys_figures, 'figure_' + com['type'].split()[1]), 'w', encoding='utf-8').write(com['type'].split()[2])
                mkdir('{}\{}'.format(values_figures, 'figure_' + com['type'].split()[1])) if not path.exists('{}\{}'.format(values_figures, 'figure_' + com['type'].split()[1])) else print('ВНИМАНИЕ! Путь <{}\{}> уже существует.\nУбедитесь в том, что пакет не конфликтует с прочими файлами'.format(values_figures, 'figure_' + com['type'].split()[1]))
                for i in range(3, len(com['type'].split()), 2): open('{}\{}\{}.txt'.format(values_figures, 'figure_' + com['type'].split()[1], com['type'].split()[i]), 'w', encoding='utf-8').write(com['type'].split()[i+1])
        except Exception as error:
            print(f'ВНИМАНИЕ! Произошла ошибка при создании фигуры: {error=}, {type(error)=}\nВозможная причина: ', end='')
    ...
    print('Пакет данных <%s> установлен' % dp.replace(f'{folder}\dp_', '').replace(f'{folder}\datapack_', '').replace('.txt', ''))
        ...
        while True:
            ...
            if a.isdigit():
                if int(a) < 1 or int(a) > len(data): print(f'Номер <{a}> не в допустимом диапазоне <1-{len(data)}> пакетов')
                    ...
                    if input('Нажмите <Enter>, чтобы подтвердить установку пакета <%s>: ' % data[int(a) - 1].replace(f'{datapacks_folder}\dp_', '').replace(f'{datapacks_folder}\datapack_', '').replace('.txt', '')) == '':
                        InstallDataPack(data[int(a) - 1])
                        break
                    else: print('Отменено')
            elif not a.isdigit():
                if a in [x.replace(f'{datapacks_folder}\dp_', '').replace(f'{datapacks_folder}\datapack_', '').replace('.txt', '') for x in data]:
                    ...
                    if input(f'Нажмите <Enter>, чтобы подтвердить установку пакета <{a}>: ') == '':
                        InstallDataPack(glob(f'{datapacks_folder}\*{a}.txt')[0])
                        break
                    else: print('Отменено')
...
if __name__ == '__main__':
    ...
    else: [InstallDataPack(x) for x in sys.argv[1:]]
