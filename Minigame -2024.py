try:  # Защита от 102-го
    ...
    def IntegrityCheck() -> 'Проверка целостности программы':  # Проверка целостности программы
        ...
        if not path.exists('figures.py'): RaiseError(causes_keys[0], 'Отсутствует файл <figures.py>')  # Проверка наличия папки. В случае ее отсутствия - вызывается ошибка
        print('Выполнено')  # Вывод информации в консоль
        ...
        if 0 in exist:  # Если есть отсутствующие пути
            ...
            RaiseError(title, amount + ', '.join(errors))  # Вызов ошибки
        print('Выполнено')  # Вывод информации в консоль
    def FindPossibleCause(error: 'Имя ошибки') -> 'Возможная причина':  # Нахождение возможной причины ошибки (принимает имя ошибки)
    ...
    def RaiseError(title: 'Оглавление ошибки', txt: 'Текст ошибки') -> None:  # Вызов ошибки (принимает имя и текст ошибки)
    ...
    def add_hk(x: 'Имя функции', figures_keys: 'Клавиша вызова функции', values: 'Что будет делать функция') -> 'Добавление функции': add_hotkey(figures_keys[x], lambda: eval(f'{x}({values[x]})'))  # Добавление горячей клавиши для управления
    ...
    def start() -> None:  # Основная программа
        ...
        if not path.exists(title_path) or title_name in {''}:  # Если путь отсутствует или оно пустое
            title_name: 'Имя окна' = 'The Minigame by Malukhin Miron -2024 ©️'  # Запись стандартного имени окна в переменную
        ...
        for i in figures: add_hk(i, figures_keys, values)  # Применение параметров фигур
        print('Выполнено')  # Вывод информации в консоль
